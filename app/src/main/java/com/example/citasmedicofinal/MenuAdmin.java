...
//IA PARA CONSULTAR SALDO Y PARAMETROS
...
public class MenuAdmin extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cajaCotizacionTotal = (TextView) findViewById(R.id.cajaCotizacionTotal);
        listview = (ListView)findViewById(R.id.listViewFiltro);


        crearSpinner();

        //MOSTRAMOS LA COTIZACION TOTAL ACTUAL
        db.collection("citas").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                for (QueryDocumentSnapshot document : task.getResult()) {
                    int total = document.getData().size()-1;
                    Log.i("TOTAL", "EL TOTAL ES " + total);
                    cajaCotizacionTotal.setText(String.valueOf(total));
                }
            }
        });
    ...
    /**
     * Evento que nos permite volver al menu o mostrar las consultas dependiendo del filtro de busqueda aplicado
     * @param v
     */
    @Override
    ...
    public void cargarDatos(){
        ...
        busquedaPaciente = new ArrayList<>();
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                ...
                if (task.isSuccessful()) {
                    for (QueryDocumentSnapshot document : task.getResult()) {
                        ...
                        filtroConsulta = new ArrayList<>();
                        filtroCorreo = new ArrayList<>();
                        filtroDias = new ArrayList<>();
                        filtroHoras = new ArrayList<>();
